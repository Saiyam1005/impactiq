        ...
        <nav className="fixed top-0 left-0 right-0 z-50 h-16 bg-bg-primary/95 backdrop-blur-md border-b border-border-subtle">
            <div className="max-w-7xl mx-auto h-full flex items-center justify-between px-4 lg:px-8">
                ...
                <Link to="/" className="flex items-center gap-2 group">
                    <HiOutlineBolt className="text-cyan text-2xl" />
                ...
                <div className="hidden md:flex items-center gap-3">
                    <form onSubmit={handleSearch} className="relative">
                        ...
                        <input
                            ...
                            className="bg-bg-card border border-border-subtle rounded-lg pl-9 pr-3 py-1.5 text-sm text-text-primary placeholder-text-muted w-44 focus:w-56 focus:border-cyan focus:outline-none transition-all duration-300"
                        />
                    </form>
                    ...
                    >
                        <div className="px-4 py-3 space-y-1">
                            ...
                            <form onSubmit={handleSearch} className="pt-2 relative">
                                <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted text-sm mt-1" />
                                <input
                                    type="text"
                                    value={search}
                                    onChange={e => setSearch(e.target.value)}
                                    placeholder="Search players..."
                                    className="w-full bg-bg-card border border-border-subtle rounded-lg pl-9 pr-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-cyan focus:outline-none"
                                />
                            </form>
                        </div>
                    </motion.div>
