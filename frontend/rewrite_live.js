...
let content = fs.readFileSync('./src/pages/LiveImpact.jsx', 'utf8');

const newLogic = `function getInitialMatchState() {
    return {
        indScore: 187,
        indWickets: 4,
        ausScore: 0,
...
const replaceRegex = /export default function LiveImpact\(\) \{[\s\S]*?(?=\/\/ Live Match Calculations)/m;
content = content.replace(replaceRegex, newLogic + '\n    // Live Match Calculations\n');
fs.writeFileSync('./src/pages/LiveImpact.jsx', content);
